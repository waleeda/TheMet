...
public enum SmithsonianClientError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidResponse:
            return "Unexpected response from the Smithsonian Open Access API."
        case .statusCode(let code):
            return "Received HTTP status code \(code) from the Smithsonian Open Access API."
        case .requestBuilderFailed:
            return "Failed to build a request for the Smithsonian Open Access API."
        }
    }
}

public final class SmithsonianClient {
    public static let shared = SmithsonianClient()
    ...
    public init(
        baseURL: URL = URL(string: "https://api.si.edu/openaccess/api/v1.0")!,
    ...
    public func search(_ query: SmithsonianSearchQuery = SmithsonianSearchQuery()) async throws -> SmithsonianSearchResponse {
        let url = try buildURL(path: "search", queryItems: query.queryItems(apiKey: apiKey))
        return try await fetch(url: url, as: SmithsonianSearchResponse.self)
    }

    public func object(id: String) async throws -> SmithsonianObject {
        let url = try buildURL(path: "content/\(id)", queryItems: [URLQueryItem(name: "api_key", value: apiKey)])
        let response = try await fetch(url: url, as: SmithsonianObjectResponse.self)
        return response.object
    }

    private func buildURL(path: String, queryItems: [URLQueryItem] = []) throws -> URL {
        guard var components = URLComponents(url: baseURL, resolvingAgainstBaseURL: false) else {
            throw SmithsonianClientError.invalidResponse
        }
    ...
    private func fetch<T: Decodable>(url: URL, as type: T.Type) async throws -> T {
        ...
        while true {
            ...
            do {
                ...
                throw SmithsonianClientError.requestBuilderFailed(error)
            }
            ...
            do {
                ...
                guard 200..<300 ~= httpResponse.statusCode else {
                    ...
                    throw SmithsonianClientError.statusCode(httpResponse.statusCode)
                }
