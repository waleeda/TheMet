...
public enum EuropeanaClientError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidResponse:
            return "Unexpected response from the Europeana Search API."
        case .statusCode(let code):
            return "Received HTTP status code \(code) from the Europeana Search API."
        case .requestBuilderFailed:
            return "Failed to build a request for the Europeana Search API."
        }
    }
}
...
public final class EuropeanaClient {
    ...
    public init(
        baseURL: URL = URL(string: "https://api.europeana.eu/record/v2/search.json")!,
        apiKey: String = "apidemo",
    ...
    public func search(_ query: EuropeanaSearchQuery) async throws -> EuropeanaSearchResponse {
        var queryItems = query.queryItems
        queryItems.append(URLQueryItem(name: "wskey", value: apiKey))
